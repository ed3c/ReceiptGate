...
export async function healthHandler() {
  return json({ ok: true, service: "receiptgate-vercel-bun" });
}

export async function demoHandler(request: Request) {
  try {
...
export async function liveComputeHandler() {
  try {
...
export async function liveAgentHandler() {
  try {
...
export async function walletChallengeHandler(request: Request) {
  try {
    const address = new URL(request.url).searchParams.get("address") ?? "";
    return json(createWalletChallenge(address));
  } catch (error) {
